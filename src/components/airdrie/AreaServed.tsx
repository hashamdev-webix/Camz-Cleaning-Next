        ...
        <div className="flex flex-col space-y-6">
          ...
          <div className="space-y-4">
            {serviceAreas.map((area, index) => (
              <div
                key={index}
                className="flex items-center gap-3 text-gray-700 leading-relaxed text-sm md:text-lg"
              >
                <IoCheckmarkCircleOutline
                  size={24}
                  className="text-[#004A8C] shrink-0"
                />
                <p>
                  <span className="font-bold text-slate-800">{area.city}</span>{" "}
                  – {area.description}
                </p>
              </div>
            ))}
