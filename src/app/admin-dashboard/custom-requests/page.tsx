  ...
  return (
    <div className="px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
      <div className="mx-auto max-w-[1500px]">
        <div className="mb-6">
          <p className="text-sm font-bold uppercase text-[#4276B2]">
            Customer enquiries
          </p>
          ...
          <p className="mt-2 text-slate-600">
            Review submitted checklists and contact customers to arrange a
            quote.
          </p>
        </div>
        ...
        {error && (
          <div className="border border-red-200 bg-red-50 p-4 text-sm text-red-700">
            Requests could not be loaded. Confirm the admin read policy has been
            added in Supabase.
          </div>
        )}
        {!error && !requests.length && (
          <div className="border border-slate-200 bg-white px-6 py-16 text-center">
            <CheckSquare2 className="mx-auto text-slate-300" size={36} />
            <h2 className="mt-4 text-xl font-bold text-slate-900">
              No requests yet
            </h2>
            <p className="mt-2 text-sm text-slate-500">
              New custom cleaning submissions will appear here.
            </p>
          </div>
        )}

        {!!requests.length && selected && (
          <div className="grid min-h-[680px] overflow-hidden border border-slate-200 bg-white lg:grid-cols-[360px_1fr]">
            <aside className="border-b border-slate-200 lg:border-b-0 lg:border-r">
              <div className="border-b border-slate-200 px-4 py-4">
                <p className="text-sm font-bold text-slate-900">All requests</p>
              ...
              <div className="max-h-[420px] divide-y divide-slate-100 overflow-y-auto lg:max-h-[720px]">
                {requests.map((request) => {
                  ...
                  return (
                    <Link
                      key={request.id}
                      href={`/admin-dashboard/custom-requests?id=${request.id}`}
                      className={`flex items-start gap-3 px-4 py-4 transition ${active ? "bg-[#EAF4FF]" : "hover:bg-slate-50"}`}
                    >
                      <span
                        className={`mt-1 h-2.5 w-2.5 shrink-0 rounded-full ${request.status === "new" ? "bg-cyan-500" : "bg-slate-300"}`}
                      />
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="mt-2 inline-block rounded-sm bg-slate-100 px-2 py-1 text-[10px] font-bold uppercase text-slate-600">
                          {request.status}
                        </span>
                      </span>
                      <ChevronRight
            ...
            <article className="min-w-0">
              <header className="border-b border-slate-200 p-5 sm:p-6">
                <div className="flex flex-col justify-between gap-5 xl:flex-row xl:items-start">
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="rounded-sm bg-cyan-50 px-2 py-1 text-xs font-bold uppercase text-cyan-800">
                        {selected.status}
                      </span>
                    </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <a
                      ...
                      className="flex min-h-11 items-center gap-2 rounded-md bg-[#0B4E9B] px-4 text-sm font-bold text-white hover:bg-[#00A8D4]"
                    >
                      <Phone size={17} /> Call customer
                    </a>
                    <a
                      href={`mailto:${selected.email}`}
                      className="flex min-h-11 items-center gap-2 rounded-md border border-slate-300 px-4 text-sm font-bold text-slate-700 hover:border-[#0B4E9B] hover:text-[#0B4E9B]"
                    >
                ...
                <div className="mt-5 grid gap-3 text-sm sm:grid-cols-2 xl:grid-cols-3">
                  <a
                    href={`tel:${selected.phone}`}
                    className="flex items-center gap-2 text-slate-700 hover:text-[#0B4E9B]"
                  >
                    <Phone size={16} /> {selected.phone}
                  </a>
                  <a
                    href={`mailto:${selected.email}`}
                    className="flex min-w-0 items-center gap-2 text-slate-700 hover:text-[#0B4E9B]"
                  >
              ...
              <div className="space-y-7 p-5 sm:p-6">
                <section>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {selected.service_types.map((service) => (
                      <span
                        key={service}
                        className="rounded-sm bg-[#EAF4FF] px-3 py-2 text-sm font-semibold text-[#0B4E9B]"
                      >
                ...
                <section>
                  ...
                  <div className="mt-3 grid gap-px overflow-hidden border border-slate-200 bg-slate-200 sm:grid-cols-2 xl:grid-cols-3">
                    ...
                    {Object.entries(selected.checklist || {}).map(
                      ([areaId, rawSection]) => {
                        ...
                        return (
                          ...
                          >
                            <h4 className="font-bold text-[#0B4E9B]">
                                  ...
                                  {section.photo_paths.map((path, index) =>
                                    photoUrls[path] ? (
                                      <a
                                        ...
                                        target="_blank"
                                        rel="noreferrer"
                                        className="flex min-h-10 items-center gap-2 rounded-md border border-slate-200 px-3 text-sm font-semibold text-[#0B4E9B] hover:border-[#0B4E9B]"
                                      >
                ...
                <section className="border-t border-slate-200 pt-5 text-sm text-slate-600">
