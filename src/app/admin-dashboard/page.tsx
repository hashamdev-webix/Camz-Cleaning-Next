...
const consoleItems = [
  ...
  { label: "Before/After", href: "/admin-dashboard/before-after", icon: Images, color: "text-blue-400" },
];
...
export default async function AdminDashboardPage() {
  ...
  return <div className="min-h-screen bg-[#020817] px-4 py-8 text-white sm:px-7 lg:px-10">
    <div className="mx-auto max-w-7xl">
      ...
      <section className="mt-10"><div className="flex items-center justify-between"><h2 className="text-2xl font-bold">Recent Timeline</h2><Link href="/admin-dashboard/bookings" className="text-sm font-bold text-[#4A86F7]">View all</Link></div><div className="mt-5 space-y-3">{allJobs.slice(0, 5).map((job) => <div key={job.id} className="flex items-center gap-4 rounded-xl border border-white/10 bg-[#0B162B] p-4"><span className="h-3 w-3 shrink-0 rounded-full bg-amber-400" /><div className="min-w-0 flex-1"><p className="truncate font-bold">{job.service_name || "Cleaning service"}</p><p className="mt-1 text-xs text-slate-500">{new Date(job.created_at).toLocaleString("en-CA", { dateStyle: "medium", timeStyle: "short" })}</p></div><span className="rounded-full bg-amber-400/10 px-3 py-1 text-xs font-bold uppercase text-amber-400">{job.status}</span></div>)}{!allJobs.length && <p className="rounded-xl border border-white/10 bg-[#0B162B] px-5 py-10 text-center text-sm text-slate-500">No booking activity yet.</p>}</div></section>
    </div>
  </div>;
}
